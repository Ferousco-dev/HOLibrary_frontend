## What this is

Screen:
Closes task:

## Screenshots

Paste both. Drag the image straight into this box.

**Desktop (1440 wide):**

**Phone (375 wide):**

## Checks

Tick each one only if you actually did it.

- [ ] I copied the frame from `pages/_template.html` and did not change it
- [ ] `aria-current="page"` is on this page's nav link, and no other
- [ ] Every colour and gap comes from a `var(--…)` token, no raw hex or pixels
- [ ] Every `<input>` has a `<label for>` that matches its `id`
- [ ] Every `<img>` has meaningful `alt` text (or `alt=""` if decorative)
- [ ] The hamburger opens and closes, and Esc closes it
- [ ] I tabbed through the whole page and could reach every control
- [ ] No text is announced by colour alone
- [ ] Data from the API goes in with `textContent`, not `innerHTML`
- [ ] I did not write my own `fetch` — I used `api.get` / `api.post`

**Lighthouse accessibility score:**  /100

## Ìlànà

Decisions recorded (DEC-):
Defects recorded (DEF-):
